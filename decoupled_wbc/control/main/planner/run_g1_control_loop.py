...
from decoupled_wbc.control.main.planner.configs.configs import ControlLoopConfig
...
def main(config: ControlLoopConfig):
    ...
    # Subscribes to CONTROL_GOAL_TOPIC
        ...
        while ros_manager.ok():
            ...
            with telemetry.timer("total_loop"):
                ...
                with telemetry.timer("observe"):
                    # Read the latest robot/simulation observation
                    obs = env.observe()
                    wbc_policy.set_observation(obs)

                # Measure policy setup time
                with telemetry.timer("policy_setup"):
                    # Pull the newest upper-body joint/pose target
