...
class G1Constraints:
    ...
    def compute_feet_poses(self, q: np.ndarray) -> tuple[pin.SE3, pin.SE3]:
    ...
    def compute_feet_positions(self, q: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    ...
    def compute_center_of_mass(self, q: np.ndarray) -> np.ndarray:
    ...
    def compute_foot_contact_points(self, q: np.ndarray) -> np.ndarray:
    ...
    def com_projection(self, q: np.ndarray) -> np.ndarray:
        """Project the center of mass onto the ground plane."""
        com = self.compute_center_of_mass(q)
        return np.array([com[0], com[1], 0.0])

    def compute_stability(self, q: np.ndarray) -> float:
        """Signed CoM distance to the nearest support-polygon edge;
        negative outside."""
        contact_points_xy = self.compute_foot_contact_points(q)[:, :2]
    ...
    def is_stable(self, q: np.ndarray, margin: float = 0.05) -> bool:
