...
from decoupled_wbc.control.main.config_template import (
    ArgsConfig as ArgsConfigTemplate,
)
...
class TrajectoryConfig(BaseConfig):
    """Configuration for replaying planner joint trajectories from a local dataset."""

    trajectory_path: str = "dataset/retar_dualarm_18.npz"
    """Path relative to the planner package directory."""

    planner_frequency: float = 20.0
    """Publishing frequency in Hz; one publish advances one trajectory frame."""

    loop_trajectory: bool = False
    """Do not loop initially because the final-to-first pose jump may be unsafe."""

    initial_transition_time: float = 2.0
    """Time used to smoothly move to the first trajectory pose."""


@dataclass
class PlannerConfig:
    """Configuration for the OMPL motion-planning ROS2 server."""

    enable_waist: bool = True
    """Whether waist joints are included in the controller upper-body group."""

    high_elbow_pose: bool = False
    """Use high-elbow default upper-body posture when mapping to the controller."""
    ...
    goal_type: Literal[
        "upper_body", "whole_body", "bimanual", "left", "right"
    ] = "upper_body"
    ...
    """Launch a MuJoCo viewer for the planning robot model."""
    ...
    trajectory_topic: str = "ControlPolicy/upper_body_pose"
    """ROS topic used to stream planned upper-body waypoints for execution."""

    execute_immediately: bool = True
    """Default for plan requests that omit execute_immediately: stream the path after planning."""
